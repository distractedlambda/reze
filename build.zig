...
const Configurator = struct {
    ...
    xorgproto_install: ?*Step.InstallDir,
    ...
    fn addXorgproto(self: *Configurator) *Step.InstallDir {
        const step = self.build.addInstallDirectory(.{
    ...
    fn addGlfw(self: *Configurator) *Step.Compile {
        ...
        if (self.target.isWindows()) {
            ...
            const xcb_lib = self.createCLib("xcb");
            xcb_lib.installHeader("third_party/libxcb/src/xcb.h", "xcb/xcb.h");
            xcb_lib.defineCMacro("HAVE_SENDMSG", null);
            xcb_lib.defineCMacro("HAVE_ABSTRACT_SOCKETS", null);
            xcb_lib.defineCMacro("XCB_QUEUE_BUFFER_SIZE", "16384");

            const xlib_conf = self.build.addConfigHeader(.{
                .style = .{
                    .autoconf = .{
                        .path = "third_party/libx11/include/X11/XlibConf.h.in",
                    },
            ...
            const xkb_lib = self.createCLib("xkb");
            xkb_lib.addIncludePath("third_party/libx11/include");
            xkb_lib.addIncludePath("third_party/libx11/include/X11");
            xkb_lib.addIncludePath("third_party/libx11/src/xcms");
            xkb_lib.addIncludePath("third_party/libx11/src/xkb");
            xkb_lib.addIncludePath("third_party/libx11/src/xlibi18n");
            xkb_lib.addIncludePath("third_party/libx11/src");
            xkb_lib.addIncludePath("third_party/xorgproto/include");
            xkb_lib.addConfigHeader(xlib_conf);
            xkb_lib.defineCMacro("_DEFAULT_SOURCE", null);
            xkb_lib.addCSourceFiles(&.{
            ...
            const x11_lib = self.createCLib("x11");

            x11_lib.linkLibrary(xcb_lib);
            x11_lib.linkLibrary(xkb_lib);

            x11_lib.addIncludePath("third_party/libx11/include");
            x11_lib.addIncludePath("third_party/libx11/include/X11");
            x11_lib.addIncludePath("third_party/libx11/src/xcms");
            x11_lib.addIncludePath("third_party/libx11/src/xkb");
            x11_lib.addIncludePath("third_party/libx11/src/xlibi18n");
            x11_lib.addIncludePath("third_party/libx11/src");
            x11_lib.addIncludePath("third_party/xorgproto/include");
            x11_lib.defineCMacro("_DEFAULT_SOURCE", null);
            x11_lib.defineCMacro("X11_t", null);
            x11_lib.defineCMacro("TRANS_CLIENT", null);
            x11_lib.defineCMacro("XKB", null);
            // FIXME: X11_CLFAGS?
            x11_lib.addConfigHeader(xlib_conf);
            x11_lib.addCSourceFiles(&.{
                "third_party/libx11/src/AllCells.c",
            ...
            const xcursor_config_header = self.build.addConfigHeader(.{
                .style = .{
                    .autoconf = .{
            ...
            lib.addConfigHeader(xcursor_config_header);
            lib.installConfigHeader(xcursor_config_header, .{});

            lib.installHeadersDirectory("third_party/libxrandr/include", "");
            lib.installHeadersDirectory("third_party/libxrender/include", "");
